...
// ── Premium ──
// Thành viên Premium mượn sách miễn phí; tài khoản thường trả BORROW_PRICE (3.000đ) / cuốn.
let isPremium = false;

function updatePremiumButton() {
  const btn = document.getElementById("premium-btn");
  if (!btn) return;
  btn.textContent = isPremium ? "👑 Premium" : "👑 Nâng cấp";
  btn.classList.toggle("is-active", isPremium);
}
...
async function loadAndRenderProducts() {
  ...
  const [snapshot, myBorrowedKeys, premiumStatus] = await Promise.all([
    get(child(dbRef, "products")),
    fetchMyActiveBorrowKeys(),
    getPremiumStatus(database, currentUserID),
  ]);
...
function createCard(product, myBorrowedKeys) {
  ...
  card.innerHTML = `
    ...
    <div class="price">${
      isPremium
        ? `<span class="price-old">${formatVND(BORROW_PRICE)}</span> Miễn phí`
        : `${formatVND(BORROW_PRICE)}<span class="price-unit"> / lượt mượn</span>`
    }</div>
